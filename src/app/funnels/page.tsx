    ...
    <div className="min-h-screen bg-black text-white p-8">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            <h1 className="text-4xl font-bold mb-2">Content Funnels</h1>
            <p className="text-gray-400">Visualize conversion paths from awareness to booking</p>
          </div>
          <Link
            href="/"
        ...
        <div className="grid grid-cols-3 gap-4 mb-8">
          <div className="bg-gradient-to-br from-blue-500/20 to-blue-600/10 border border-blue-500/30 rounded-xl p-6">
            <div className="flex items-center gap-3 mb-2">
              ...
              <span className="text-sm text-gray-400">TOFU Content</span>
            </div>
            <div className="text-3xl font-bold">{funnelContent.TOFU.length}</div>
            <div className="text-sm text-gray-400 mt-1">Awareness posts</div>
          </div>
          <div className="bg-gradient-to-br from-amber-500/20 to-amber-600/10 border border-amber-500/30 rounded-xl p-6">
            <div className="flex items-center gap-3 mb-2">
              <TrendingUp className="w-5 h-5 text-amber-400" />
              <span className="text-sm text-gray-400">MOFU Content</span>
            </div>
            <div className="text-3xl font-bold">{funnelContent.MOFU.length}</div>
            <div className="text-sm text-gray-400 mt-1">Consideration posts</div>
          </div>
          <div className="bg-gradient-to-br from-green-500/20 to-green-600/10 border border-green-500/30 rounded-xl p-6">
            <div className="flex items-center gap-3 mb-2">
              <Target className="w-5 h-5 text-green-400" />
              <span className="text-sm text-gray-400">BOFU Content</span>
            </div>
            <div className="text-3xl font-bold">{funnelContent.BOFU.length}</div>
            <div className="text-sm text-gray-400 mt-1">Conversion posts</div>
          </div>
        </div>

        {/* Detected Funnels */}
        <div className="mb-8">
          <h2 className="text-2xl font-bold mb-4">Detected Funnels</h2>
          ...
          {funnels.length === 0 ? (
            <div className="bg-white/5 border border-white/10 rounded-xl p-12 text-center">
              ...
              <h3 className="text-xl font-semibold mb-2">No funnels detected yet</h3>
              <p className="text-gray-400 mb-6">
        ...
        <div>
          <h2 className="text-2xl font-bold mb-4">All Content by Funnel Stage</h2>
              ...
              <div key={stage} className="space-y-3">
                ...
                <div className="space-y-2">
                  ...
                  {funnelContent[stage].length === 0 && (
                    <div className="text-sm text-gray-600 italic p-4">No content yet</div>
                  )}
                </div>
              </div>
            ))}
...
function FunnelVisualization({ funnel, index }: { funnel: { id: string; stages: ContentItem[][] }; index: number }) {
  ...
  return (
    <div className="bg-gradient-to-br from-white/5 to-white/10 border border-white/20 rounded-xl p-6">
      <h3 className="text-lg font-semibold mb-6">Funnel #{index + 1}</h3>
      ...
      {bofu[0]?.conversionGoals && bofu[0].conversionGoals.length > 0 && (
        <div className="mt-4 pt-4 border-t border-white/10">
          <div className="text-sm text-gray-400 mb-2">Conversion Goal:</div>
          <div className="flex flex-wrap gap-2">
            {bofu[0].conversionGoals.map((goal, idx) => (
              <div key={idx} className="px-3 py-1 bg-green-500/20 border border-green-500/30 rounded-full text-sm">
